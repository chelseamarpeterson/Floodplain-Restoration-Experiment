...
plot.length.m = 48 #m 
plot.width.m = 12 #m 
plot.area.m2 = plot.length.m * plot.width.m #m2
plot.area.ha = plot.area.m2 / m2.per.ha #ha
plot.length.ft = plot.length.m * ft.per.m #ft

# treatment labels
trt.letters = c("A","B","C","D","E","R")
trt.names = c("Balled-and-burlapped","Bareroot","Seedling","Acorn","Seedbank","Reference")
n.t = length(trt.letters); n.p = 3
...
cwd.data$full.treatment.name = rep(0, nrow(cwd.data))
...
fwd.counts = cwd.data[which(is.na(cwd.data$diameter.cm)), 
                      c("treatment","full.treatment.name","plot","fwd.count")]
...
fwd.counts$fwd.vol = (1/plot.length.ft) * (C1*(pi^2)/8) * fwd.counts$fwd.count * ((med.fwd.diam.cm/cm.per.in)^2) #m3/ha
fwd.counts$int.fwd.carbon = fwd.counts$fwd.vol * cwd.density.decay.class3 * fwd.C.content  # m3/ha * g/cm3 * g/g = Mg/ha
...
cwd.diams$cwd.vol = (1/plot.length.ft) * (C1*(pi^2)/8) * (cwd.diams$diameter/cm.per.in)^2 #m3/ha
cwd.diams$cwd.carbon = cwd.diams$cwd.vol * cwd.density.decay.class3 * cwd.diams$taxon.c.content  # Mg/ha
...
snag.dbh.data$full.treatment.name = rep(0, nrow(snag.dbh.data))
...
snag.diams$basal.area = (pi * (snag.diams$dbh.cm/2)^2) / plot.area.m2 #m2/ha = cm2/m2
snag.diams$vol.min = snag.diams$basal.area * (min.snag.height.ft / ft.per.m) #m3/ha
...
snag.diams$carbon.min = snag.diams$vol.min * snag.density.decay.class3 * snag.diams$taxon.c.content  # Mg/ha
...
snag.counts$dead.stem.carbonmin = snag.counts$dead.stem.volmin * snag.density.decay.class3 * snag.counts$taxon.c.content  # Mg/ha
...
for (i in 1:n.t) {
  for (j in 1:n.p) {
    ...
    if (length(treatment.plot.id) == 0) {
      zero.row = data.frame(matrix(nrow=1, ncol=4))
      colnames(zero.row) = c("treatment","full.treatment.name","plot","dead.stem.carbonmin")
      zero.row[1, c("treatment","full.treatment.name","plot")] = c(trt.letters[i], trt.names[i], j)
...
all.stem.data$abg = get_biomass(dbh = rep(med.fwd.diam.cm, nrow(all.stem.data)),
                                genus = all.stem.data$genus, 
                                species = all.stem.data$spp, 
                                coords = c(-90.1835, 41.5542)) * all.stem.data$stem.count / kg.per.Mg # [kg -> Mg]
...
all.stem.data$tot.live.stem.carbon = all.stem.data$abg.live.stem.carbon + all.stem.data$bg.live.stem.carbon # Mg C/ha

# add full treatment name
all.stem.data$full.treatment.name = rep(0, nrow(all.stem.data))
...
snag.frax.pen$hyp.abg.bm.live = get_biomass(dbh = snag.frax.pen$dbh.cm, 
                                            genus = snag.frax.pen$genus, 
                                            species = snag.frax.pen$spp, 
                                            coords = c(-90.1835, 41.5542))/kg.per.Mg
snag.frax.pen$hyp.abg.bm.ha = snag.frax.pen$hyp.abg.bm.live / plot.area.ha # Mg/ha

# use Chojnacky allometric equations to calculate belowground to aboveground biomass ratio
snag.frax.pen$r = exp(allo.df[1,"cr.b0"] + allo.df[1,"cr.b1"] * log(med.fwd.diam.cm)) # ratio = exp(b0 + b1*log(dbh [cm]))
snag.frax.pen$hyp.bg.bm.ha = snag.frax.pen$hyp.abg.bm.ha * snag.frax.pen$r # Mg/ha
snag.frax.pen$hyp.bm.ha = snag.frax.pen$hyp.abg.bm.ha + snag.frax.pen$hyp.bg.bm.ha # Mg/ha

# estimate c content and multiply by biomass to get C stock
snag.frax.pen$taxon.c.content = rep(0,nrow(snag.frax.pen))
...
snag.frax.pen$hyp.Cstock.ha = snag.frax.pen$hyp.bm.ha * snag.frax.pen$taxon.c.content

# average by plot
snag.frax.sum = snag.frax.pen %>% 
                group_by(treatment, full.treatment.name, plot) %>% 
                summarize(snag.frax.live.c = sum(hyp.Cstock.ha)) 
...
colnames(snag.frax.dead) = c("treatment","plot","snag.frax.dead.c")
...
colnames(C.stock.data)[4:6] = c("aboveground.woody.c.stock","belowground.woody.c.stock","total.woody.c.stock")
...
colnames(understory.wide)[3:8] = c("herbaceous.biomass.c.stock","mixed.biomass.c.stock","p.arundinacea.c.stock",
                                   "h.japonicus.c.stock","herbaceous.litter.c.stock","fine.woody.debris.c.stock")
...
all.bm.data = right_join(all.bm.data,
                         live.stem.sum[,c("treatment","full.treatment.name","plot",
                                          "abg.live.stem.carbon","bg.live.stem.carbon","tot.live.stem.carbon")], 
                         by=c("treatment","full.treatment.name","plot"))

# hypothetical C stocks of Frax pen
all.bm.data = right_join(all.bm.data,
                         snag.frax.sum[,c("treatment","full.treatment.name","plot",
                                          "snag.frax.live.c","snag.frax.dead.c")], 
                         by=c("treatment","full.treatment.name","plot"))
