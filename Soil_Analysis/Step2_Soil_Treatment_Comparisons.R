...
soil.df$treatment = as.factor(soil.df$treatment)
soil.df$strip = as.factor(soil.df$strip)
soil.df$plot = as.factor(soil.df$plot)
all.means = list()
all.lists = list()
for (i in 1:n.v) {
  v = vars[i]
  all.means[[v]] = mean(soil.df[,v])
  all.lists[[v]] = list(treatment = soil.df$treatment,
                        strip = soil.df$strip,
                        plot = soil.df$plot,
...
for (i in 1:n.v) {
  ...
  for (j in 1:1) {
    ...
    #rhat.df[which(rhat.df$variable == v.i & rhat.df$model == m.j),] = model.df.ij
...
n.c = length(criteria)
#comp.df = read.csv("Soil_Analysis/Posteriors/Soil_Model_Information_Criteria.csv")[,1:13]
#comp.df = comp.df[-which(comp.df$variable == "tic.percent"),]
...
#df.int = read.csv("Soil_Analysis/Posteriors/Soil_Posterior_Intervals_ChainCount5_BRMS.csv")
